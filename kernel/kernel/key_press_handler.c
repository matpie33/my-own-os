...
bool is_displayable_character (uint8_t scancode){
	return (scancode >=0x02 && scancode <= 0x0D) || (scancode >= 0x10 && scancode <= 0x1B) || (scancode >= 0x1E && scancode <= 0x29) 
	|| (scancode >= 0x2B && scancode <= 0x35) || scancode == 0x37 || scancode == 0x39 || (scancode >= 0x47 && scancode <= 0x53);
}
